...
def start1_one(request,checklist_id,saved_form1):
    ...
    CheckListTransHeaders = CheckList_Header.objects.get(CheckList_ID=checklist_id)  #Pipe Checklist
    CheckListTransHeaders1 = CheckList_Trans_Header.objects.get(Doc_ID=saved_form1)   # Doc ID: 30, CheckList ID: Pipe Checklist
    ...
    if request.method == 'POST':
            ...
            if all_forms_valid:
             messages.info(request, 'Checklist submitted to DHEQA successfully..!')
             return redirect('/home')
...
def DHPre_one(request, pk):    
    ...
    if request.method == 'POST':
        ...
        if len(valid_forms) == len(prefixes):
            ...
            messages.info(request, 'Checklist submitted to Project_QA Successfully..!')
            return redirect('/home')
        ...
        #-----trying---
     # Handling the checklist rejection process
    if request.method == 'POST' and 'reject' in request.POST:
        ...
        if formR.is_valid():
            ...
            checklist.save()
            messages.info(request, 'Checklist rejected.')
            return redirect('/home')
    else:
        formR = rejected_from_ProjectQAForm(instance=checklist)
    context = {
        'form1': form1,
...
def DHEReject(request,pk):
    #checklist = CheckList_Trans_Header.objects.get(pk=41)
    checklist = get_object_or_404(CheckList_Trans_Header,Doc_ID=pk)
    if request.method == 'POST':
        formR = rejected_from_ProjectQAForm(request.POST,instance=checklist) 
        if formR.is_valid():
            formR.save(commit=False)
...
def all_checklists(request):
    ...
    for checklist in checklists:
     if checklist.Poorly_implimented_Quality_Checks is not None and checklist.Poorly_implimented_Quality_Checks != "":
        checklist.append(checklist.Doc_ID) 
    context ={'checklists':checklists,
            'isActive': True,
            'checklist':checklist,
            'page_title': 'Ashoka Buildcon Limited',
